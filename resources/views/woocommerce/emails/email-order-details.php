...
<h2 class="<?php echo esc_attr( $heading_class ); ?>" style="<?php echo esc_attr( $email_text_style ); ?>">
	<?php
	if ( $email_improvements_enabled ) {
	...
	if ( $email_improvements_enabled ) {
		echo '<br><span style="' . esc_attr( $email_text_style ) . '">';
	}
...
<div style="margin-bottom: <?php echo $email_improvements_enabled ? '24px' : '40px'; ?>;">
	<table class="td font-family <?php echo esc_attr( $order_table_class ); ?>" cellspacing="0" cellpadding="6" style="width: 100%;" border="1">
		...
		<thead>
			<tr>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;<?php echo esc_attr( $email_text_style ); ?>"><?php esc_html_e( 'Product', 'woocommerce' ); ?></th>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;<?php echo esc_attr( $email_text_style ); ?>"><?php esc_html_e( 'Quantity', 'woocommerce' ); ?></th>
				<th class="td" scope="col" style="text-align:<?php echo esc_attr( $text_align ); ?>;<?php echo esc_attr( $email_text_style ); ?>"><?php esc_html_e( 'Price', 'woocommerce' ); ?></th>
			</tr>
		</thead>
		<?php } ?>
		<tbody>
		...
		<tfoot>
			...
			if ( $item_totals ) {
				...
				foreach ( $item_totals as $total ) {
					...
					<tr class="order-totals order-totals-<?php echo esc_attr( $total['type'] ?? 'unknown' ); ?><?php echo esc_attr( $last_class ); ?>">
						<th class="td text-align-left" scope="row" colspan="2" style="<?php echo ( 1 === $i ) ? 'border-top-width: 4px;' : ''; ?><?php echo esc_attr( $email_text_style ); ?>">
							<?php
						...
						<td class="td text-align-<?php echo esc_attr( $order_total_text_align ); ?>" style="<?php echo ( 1 === $i ) ? 'border-top-width: 4px;' : ''; ?><?php echo esc_attr( $email_text_style ); ?>"><?php echo wp_kses_post( $total['value'] ); ?></td>
					</tr>
					<?php
				}
			}
			if ( $order->get_customer_note() && ! $email_improvements_enabled ) {
				?>
				<tr>
					<th class="td text-align-left" scope="row" colspan="2" style="<?php echo esc_attr( $email_text_style ); ?>"><?php esc_html_e( 'Note:', 'woocommerce' ); ?></th>
					<td class="td text-align-left" style="<?php echo esc_attr( $email_text_style ); ?>"><?php echo wp_kses( nl2br( wc_wptexturize_order_note( $order->get_customer_note() ) ), array() ); ?></td>
				</tr>
				<?php
			}
			if ( $order->get_customer_note() && $email_improvements_enabled ) {
				...
				<tr class="order-customer-note">
					<td class="td text-align-left" colspan="3" style="<?php echo esc_attr( $email_text_style ); ?>">

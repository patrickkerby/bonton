...
<button class="btn btn-default" onclick="printDiv('order-{{ $order_number }}','receiptPrint')"><i class="fa fa-print" aria-hidden="true"></i> Print Receipt</button>
    ...
    <p class="date">
      <strong>{{ $date_selector_date }}</strong> <br>
        @if($details->has_shipping_method('flat_rate'))
          {!! $order_location !!} - {!! $shipping_data_name !!}
        @endif
        @if($timeslot)
          {{ $timeslot }}
        @endif
